<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.zuoyefengmian" label="作业封面" prop="zuoyefengmian">
          <file-upload
          tip="点击上传作业封面"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.zuoyefengmian" label="作业封面" prop="zuoyefengmian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.zuoyefengmian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.zuoyeneirong = this.ruleForm.zuoyeneirong.replace(reg,'../../../springbootm55rv/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
