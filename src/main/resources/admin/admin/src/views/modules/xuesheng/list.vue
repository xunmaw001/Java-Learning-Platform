<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('xuesheng','成绩') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="chengjitongjiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'成绩':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','成绩') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="chengjitongjiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'成绩':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','成绩') && contents.tableBtnIcon == 0" type="success" size="mini" @click="chengjitongjiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'成绩':'' }}</el-button>
    ...
    <chengjitongji-cross-add-or-update v-if="chengjitongjiCrossAddOrUpdateFlag" :parent="this" ref="chengjitongjiCrossaddOrUpdate"></chengjitongji-cross-add-or-update>
...
import chengjitongjiCrossAddOrUpdate from "../chengjitongji/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      chengjitongjiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    chengjitongjiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.chengjitongjiCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.chengjitongjiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.chengjitongjiCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
